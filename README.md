...
`npm test` runs every gate in turn, cheapest first, and stops at the first one that goes red, naming it. It needs WebKit and Firefox as well as Chromium, because the device check also loads the library and the first chapter in those two engines. The screenshots land under `out/` for a person to look at, because a green gate proves the pages load, not that they are beautiful. `docs/policies/gates.md` lists every gate with what it does and does not prove.
