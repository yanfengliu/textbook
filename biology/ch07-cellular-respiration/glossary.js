...
export const GLOSSARY = {
  ...
  ubiquinone: { term: 'Ubiquinone', def: 'A small lipid-soluble carrier, also called coenzyme Q, that shuttles electrons within the inner membrane rather than along it. Its long hydrocarbon tail keeps it in the bilayer, which is how two complexes that never touch each other exchange electrons.' },
  ...
  fermentation: { term: 'Fermentation', def: 'Any pathway that regenerates NAD<sup>+</sup> by handing electrons to an organic molecule the cell made itself, so that glycolysis can keep running with no chain and no outside acceptor. It yields no ATP of its own: the two from glycolysis are the whole of the return.' },
  ...
  'anaerobic-respiration': { term: 'Anaerobic respiration', def: 'Respiration with a chain, a gradient and a synthase, ending on something other than oxygen — nitrate, sulfate, iron(III) or carbon dioxide. It is not fermentation: the machinery is the same as the aerobic kind, and the yield is lower only because the acceptor sits higher on the ladder.' },
  ...
  uncoupler: { term: 'Uncoupler', def: 'A substance that lets protons back across the inner membrane without passing through ATP synthase, so the chain runs faster than ever and no ATP is made. All of the energy leaves as heat, which is what makes uncouplers lethal as slimming drugs and useful to a hibernating animal.' },
  'uncoupling-protein': { term: 'Uncoupling protein', def: 'A protein in the inner membrane of brown fat mitochondria that provides a controlled proton leak. It turns the gradient straight into heat on demand, and it is how a newborn mammal warms itself without shivering.' },
