...
- With chlorophyll a alone shown, **the action points run visibly broader than its absorption**: the alga makes more oxygen at 470 and 640 nm than chlorophyll a's absorption can account for. Showing chlorophyll b and the carotenoids closes the gap. That gap is the figure's argument and it must be legible at 390 px.
...
- **Flash train** — as built, twelve single flashes, one a second, each firing both photosystems, at a system rested in the dark. After darkness three clusters in four rest one step along (Kok's model, and §6.4 now says so), so the train opens with the cluster at one: nothing on flashes 1 and 2, the first oxygen on flash 3, then peaks on 7 and 11 that blur as the population drifts out of step. A note under the chart gives the reason in one sentence. By hand, **Reset** puts the cluster at zero, so three photons give no oxygen and the fourth does; the two modes' starting counts differ, and the stage now names each for what it is — the reset as a cluster holding no charge, and "dark" only for the train (the accuracy review's figure pass, finding 13).
...
- **Run** and pause, and **Reset**, to a cluster holding no charge with nothing fired.
...
- **No chapter draws the rotary motor.** Chapter 7's first plan had an `atp-synthase` figure with a `cSubunits` control so that chapter 6 could mount it at 14, and chapter 6 declined. Chapter 7 then cut that figure on 2026-09-24: its four figures are `glycolysis`, `krebs`, `respiratory-chain` and `fermentation`, and none is a rotary motor. Since `proton-ledger` was cut, chapter 6 draws no synthase at all. Nothing here is shared with chapter 7.
