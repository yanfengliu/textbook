...
**On a flat stage below 800 px.** The frame gives its flat box from an 800 px window up, while the rail keeps the stage at 480 to 782 px, so below 800 px the stage can be wider than tall. The tube in the top half of a box that short squeezed its scale's numbers into one another. So there the tube, the molecules and the table stand side by side, each the full height: the tube 146 to 170 px wide, which its longest band name needs, the molecules 100 to 130, and the table the rest, 186 px at an 800 px window and 434 at 1150. From a 960 px window the table shows everything. At 800 and 860 it gives up whole parts in the order above. Under 250 px, at those two, it also gives up its density column, which the tube's scale beside it carries. At 800 a table still too tall gives up the schemes ruled out, heated or not, and last its title and the head over the strands, which the tube's heading, the scheme stepper and the rows' own names already say. The semiconservative title names the scheme alone there, the only one too long for the column.
...
**On a flat stage below 800 px.** The frame gives its flat box from an 800 px window up, while the rail keeps the stage at 480 to 782 px, so below 800 px the stage can be wider than tall. An upright fork in a box that short was cut to a strip, and its table lost rows and sentences under the controls. So there the fork keeps its desktop drawing, lying along the stage, and the table stands beside it, 236 px at an 800 px window and 300 at 1150, its rows in two columns over the sentences. The controls wrap as one line, and the two made-up rules take short labels, Same way and Either end, so the fork scene's controls take two rows from a 960 px window. At 800 and 860 they take three, which leaves the drawing and the table 80 and 106 px. There the table gives up its titles, then its rows, and last the rule's sentence, which the pressed control names; the sentence on what the reader has done stays whole. A drawing that short closes its arms up and puts a strand's name inside the fork when there is no room outside it. The whole-chromosome scene draws its desktop picture beside a table of one column. Under 200 px of height the chromosome closes up: the enlargement is drawn only with room for its name, and the clock only where it fits, the table's elapsed time standing for it. Whatever the stage, the table takes the phone's shorter wordings before it gives up a sentence: at a 989 px stage a made-up rule's long sentence had left no room for what an enzyme taken away had done.
