...
- **`test/provenance.test.js`** — the other half of the script rule, and its gate. Two claims about a chapter page's prose, and the second is the one that closes the hole the first could not see. **First, every 「…」 run that is verbatim 通鑑 must sit inside `lang="zh-Hant"`.** **Second, a run of 通鑑's own words may not be printed bare: a maximal run of the prose that is a verbatim substring of `tongjian/corpus.js` and holds ≥ 6 Han characters must carry the mark, bracketed or not.** The floor is measured rather than chosen — across the three chapters the corpus's maximal runs are 9 Han → 1, 7 → 1, 6 → 3, 5 → 2, 4 → 14, 3 → 35, 2 → 264, so 6 is the lower edge of the clause population and 5 is the band the floor is set above — and the check prints those populations on every run. **Its bound is stated with its numbers: a 通鑑 run the book prints in Simplified is not caught**, because comparing in script-normalised space was measured and cannot ship (at floor 6 it fires 12 times and 6 of those are the book's own Simplified narration; at floor 10 the two longest are still the book's own prose) — the book's line between its own words and 通鑑's *is* the script, since a quotation is defined as verbatim character for character. The contents page must carry the key; and each `src/figures/zj-*.js` must export **`QUOTED_FIELDS`**, the names of the object fields that hold whole 通鑑 句, checked in both directions — a field holding one must be declared, and a declared field must hold one — together with `lang: 'zh-Hant'` on the element the module builds for it. **That last part is a contract this book adds to its own three figures and not to the shared figure contract** ([textbook.md](textbook.md)): a figure that prints 通鑑 has to say which of its fields is the book's, and a figure of the biology book has no 通鑑 in it to declare. A new Chinese figure that holds 通鑑's words in its data and does not declare them fails this gate.
