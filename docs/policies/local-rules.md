...
Owner directive, 2026-09-23. The owner read the project audit ([7_project-audit](../work/7_project-audit/plan.md)) and replied, in their words: *"All reasonable. Make it known to the agents and persist this in docs."* Point 10 is the scope decision they gave the book's coordinator the same day. Their answer of 2026-09-24 lifted its stop for the chapters already written, and the coordinator set those chapters' scope. These ten bind every agent here:
...
10. **Publish what is written, and start nothing new.** Chapters 7 and 8 were written before chapter 6 landed. They ship at chapter 6's reduced scope: four figures each, questions instead of figure tasks, point 4's review before they land, and no new gates. A defect the owner reports is the exception: it still gets the check for its whole class that R17 requires. No chapter after 8 starts, and other new work waits, until the owner asks.
...
- **Accepted.** "All reasonable" accepted the audit's recommendations (synthesis 5). Extra usage, one of the decisions left pending, was not decided: it stays the owner's own lever. Point 10 as first given overrode synthesis 5's one-week test, which wanted chapters 6–7 on main. Its 2026-09-24 amendment publishes chapter 7 too.
...
- **5.** The study system has never been used (F4). All 233 review-bank multiple-choice items were written with the right answer first, and Today showed them in that order (F5). Since `407827c`, Today and the chapter checks draw the order (`src/components/choice-order.js`). No page has an export (F29), so a record kept on a phone cannot reach `npm run progress:import`. The export waits on the owner, like all other new work (point 10).
...
- **10.** The owner told the book's coordinator: *"Reduce scope and stop at your current chapter's completion"*. What they chose for chapter 6, and where chapters 7 and 8 stand, is recorded at the top of [2_rest-of-the-book/plan.md](../work/2_rest-of-the-book/plan.md). It supersedes the standing goal of writing the whole book. Point 4's review was added to chapter 6 afterwards. On 2026-09-24, after chapter 6 had landed, the owner told the coordinator: *"If chapters are written then of course publish them."* Chapters 7 and 8 were the written ones, waiting on branches. Nothing after chapter 8 had been started. The coordinator set the two chapters' scope to chapter 6's, as the point says. The coordinator read the answer as covering written chapters only, so the stop still holds for anything new. The exception for a defect the owner reports answers finding 10 of [the review of this direction](../work/2_rest-of-the-book/reviews/2026-09-24-direction-review.md): R17 wants a check for each class of defect the owner reports, and a local rule may not weaken the canon (R13).
