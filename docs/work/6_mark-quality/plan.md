...
That message quotes the key page's dark-theme measurement wrongly: it reads "rgb(233,228,218) against rgb(163,228,218)... in dark". The measured values are the run at `rgb(233, 228, 218)` against its `.dek` at `rgb(163, 157, 147)`, and the light-theme pair is the run at `rgb(29, 26, 26)`... no: light is the run at `rgb(29, 26, 23)` against the dek's `rgb(92, 85, 77)`. Both come from the worker's probe (`out/probe-key-mark.mjs`) and both are in the proof entry's own words; the commit subject and the code are unaffected. A pushed message cannot be rewritten without a force-push, which this repository does not do without the owner's word, so the correction lives here where the round's record is read.