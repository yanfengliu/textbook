...
Status: **published; three revision rounds landed, the third one committed as `e400821` — the only paths this round still holds uncommitted are the ones other sessions are editing in the same tree**
...
**Published.** The book was published as `e6daf7a` and pushed, and is live at <https://yanfengliu.github.io/textbook/tongjian/>. The commits that carry the revision work after it are `0482dde` (the 原文 paired with its translation, four sections a chapter, and the colour system), `f5bd0e9` (the fleet canon) and `e400821` (the third round — the four defects the owner and two reader-proxies found, and the checks that now hold them). Since then the owner, two reader-proxies, `npm run devices` and an independent text review have produced **ten** defects — six of them things no gate was asking about — and the work that fixes them is **committed**, in `0482dde` and `e400821`:
...
**Committed, in `e400821`** (verified with `git show --stat e400821`, not carried over from the working tree): the width ladder in `tongjian/zj.css` (698 px fixed, 36% of a 1920 px screen, to 66% at 1280, 58% at 1440 and 53% at 1920), the colour round two (the date gold, the 青 on a textual note, and the four house colours through the glossary's `data-state` hook), the `All sorted.` override scoped to this book, the 字詞 list's index by first character, the two chapter-2 years the colour round had left unmarked, and — in the same commit — the tool checks this plan used to list as uncommitted: the **leftmost** header control in its corner and the 50% floor on the reading column and the 原文/譯文 block at 1280–1920 px in `tools/devices.js`, the colour-hook cascade check and its count floor in `tools/shot.js`, and the font census in the same file. The commit carried 35 files and 4,578 insertions; the three paths this plan named, `tongjian/zj.css` (+667), `tools/devices.js` (+231) and `tools/shot.js` (+762), account for 1,660 of them.

**Uncommitted in the working tree, after `e400821`:** only the paths the commit message names as other sessions' work, plus the edits those sessions have made since — that tree kept moving while this line was being written, so `git status --porcelain` is the list and this is its shape. The publication session's: `.github/workflows/pages.yml`, `tools/pages-exclude.txt`, `tools/pages-exclude.js` (untracked, so the new one matcher), `tools/subpath.js`, `test/publish.test.js` (untracked). The biology session's: `biology/ch03-cells/index.html`, the untracked `biology/ch04-membranes-and-transport/`, `src/figures/cytoskeleton.js`, `registry.js`, the untracked chapter-4 figure modules beside them under `src/figures/`, `src/palette.js`, `tools/check-content.js`, `test/check-content.test.js`, `test/membrane-table.test.js`, and `docs/design/textbook.md`, `docs/design/chapter-recipe.md`, `docs/design/figures-template.md`. The shared records two sessions are editing: `AGENTS.md`, `docs/policies/local-rules.md`, `docs/learning/lessons.md`, `docs/work/2_rest-of-the-book/`. And `tools/shot.js` and `tools/devices.js` carry edits landed since the commit by more than one session in this shared tree. **Nothing on that list is this round's, and this round's own files are clean**: `git status --porcelain -- tongjian/` prints nothing, so the book's pages, stylesheet, corpus, lexicon and card data are exactly what `e400821` holds.

**Left behind deliberately, and named in the commit message:** the other sessions' uncommitted paths, which `e400821` did not add, revert or commit — the publication work, the biology work, and the two shared records. The commit states the same list in its own words under *"Which are not, and are left uncommitted"*, and those paths are what the round's rule of an explicit `git add` of named paths only kept out of it.

**Open, and measured rather than assumed:**

- **The owner's account of the header's top-left contents button doing nothing on a phone.** Re-measured for this plan: a touch tap on `.tb-navtoggle` opens the drawer at 320, 360, 390 and 430 px on both books, on the working tree and on the published site — button 34×34 px, `aria-expanded=true`, drawer on screen, scrim shown, page locked (`out/defectdoc/probe-navtoggle-shapes.js`, `out/zj-scratch/probe-published-toggle.js`). The explanation the record supports is the target's **size**: at 320 px the header squeezed the icon buttons to **19 px** against a 24 px minimum, which `4c4acb8` fixed with `flex: 0 0 auto` and a 2.1 rem square; which build the owner was reading is not in the record. The corner is what was genuinely uncovered — the check pinned the rightmost control only, and the leftmost one's corner is new in this round, committed in `e400821`.
...
- **Three English strings remain in the shared stylesheets** — `content: "Key idea"` (`components.css`), `content: "Chapter "` and `content: "Chapter"` (`layout.css`) — and `test/strings.test.js` is the check that reads them: it extracts every quoted string in a `content:` declaration from the stylesheets a page loads, and requires each Simplified book to state a replacement rule for the selectors its pages reach, failing with the selector and the rule to add, and failing when the extraction found no literal or no override at all, so a scan that read nothing cannot read as a clean book. `All sorted.` is replaced for this book alone, at the end of `zj.css`.
