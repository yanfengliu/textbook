...
| Symptom | Root cause | Now checked by |
|---|---|---|
| A mark glued to a term's end starts the next line | a line may break after the term's `<button>` | `term.js` wraps the term and the run glued to its end (the mark, or a plural's "s") in a `<tb-term-hold>`, which `src/styles/components.css` sets `white-space: nowrap`, with a zero-width space after it and wrapping restored inside the button and the card. `test/term-hold.test.js` holds that markup and those rules. Proved red twelve ways when it landed (5 in the stylesheet, 7 in `term.js`) and seven more for the no-break space below: [gate-proofs.md, *unit: a mark written against a term's end, or a no-break space and what it glues on, is wrapped with the term, once*](gate-proofs.md#unit-a-mark-written-against-a-terms-end-or-a-no-break-space-and-what-it-glues-on-is-wrapped-with-the-term-once-testterm-holdtestjs) |
