...
| Symptom | Root cause | Now checked by |
|---|---|---|
| A mark glued to a term's end starts the next line | a line may break after the term's `<button>` | `term.js` wraps the term and the run glued to its end (the mark, a plural's "s", or a no-break space and what it glues on) in a `<tb-term-hold>`, which `src/styles/components.css` sets `white-space: nowrap`, with a zero-width space after it and wrapping restored inside the button and the card. `test/term-hold.test.js` holds that markup and those rules. Proved red twelve ways when it landed (5 in the stylesheet, 7 in `term.js`) and seven more for the no-break space below: [gate-proofs.md, *unit: a mark written against a term's end, or a no-break space and what it glues on, is wrapped with the term, once*](gate-proofs.md#unit-a-mark-written-against-a-terms-end-or-a-no-break-space-and-what-it-glues-on-is-wrapped-with-the-term-once-testterm-holdtestjs) |
| The class: where any line breaks | a break depends on the width, the text and the face | **no gate.** After the fix the probe measured 0 stranded at 390 and 320 px and at every width from 320 to 1440 px, in biology ch01–08 and 資治通鑑 ch01–03, and 0 of 608 other term paragraphs broke differently from main. That probe did not count a no-break space as glue, so it never looked at the 14 terms written `</tb-term>&nbsp;—`: chapter 8's "phosphodiester bond" and "deamination", which review finding S1 found, and 12 in chapters 3–6 that the polish merge brought in. Before `36cbc2f`, 12 of the 14 began a line with the space and the dash somewhere from 320 to 1440 px (ch03 3, ch04 3, ch05 2, ch06 2, ch08 2), chapter 8's two at 52 and 41 of the 1,121 widths, as the review measured. Re-measured on `36cbc2f` with glue counted, in Chromium: 0 of the 232 glued runs in biology ch01–08 and 0 of the 65 in 資治通鑑 ch01–03 stranded, at every width from 320 to 1440 px in the light theme and at 390 and 320 px in both themes. Chapter 7's text changes again when `figfix-ch07` lands, and it is owed the measurement then. Nothing repeats it, because this round adds no gates (chapter 6's reduced scope, which chapters 7 and 8 ship at) |

**The lesson underneath.** A line-break defect lives at every width, and a frame shows one. The 390 px frame showed 4 of the 172 pairs that strand somewhere, and none of the 12 no-break joins: 2 of them stranded at 320 px and none at 390.
