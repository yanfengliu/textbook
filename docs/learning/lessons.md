...
## 2026-09-25 — a pinned figure can start moving when it scrolls into view, and describe() need not show it

Figure 3.3 (`prokaryote`) swam on `?t=0` once it was scrolled into view: its `schedule()` did not look at `ctx.pinnedTime`, so a `setVisible(true)` that came after the frame's `setTime` restarted its loop. `npm run pinned` passed it, for two reasons. It loads each figure on the lab page and scrolls it into view straight away (`tools/pinned.js:403`), and a figure in view from the start need never get a `setVisible(true)` after `setTime`; whether the gate's pages ever get that order was not measured. And it judges a figure by `describe()` alone.
Measured: a GPU scratch probe loaded chapter 3 on `?t=0` at 390, 1024 and 1440 px, with the gram-positive and the archaeal envelope, scrolled the figure into view and read the canvas's bytes and `describe()` 1.5 s apart. Before the fix the canvas changed in all six cases, and every field of `describe()` held in all six. After `58a2c72` all six canvases hold still. Found as N1 of `docs/work/2_rest-of-the-book/reviews/2026-09-25-land-4-review-3-cell.md`, from the nucleoid's click spot moving.
Gate, owed: in `npm run pinned`, after the pinned mount, scroll each figure out of view and back, so that `setVisible(false)` and then `setVisible(true)` arrive after `setTime`. Then read the stage's pixels as well as `describe()` across the untouched window. The scroll alone would not have caught this figure, because its `describe()` held still while it swam; proving the gate red means reverting `58a2c72`'s guard.

## 2026-09-25 — `git worktree remove` deletes through a `node_modules` junction

A worktree's `node_modules` here is a junction to the main checkout's. `git worktree remove` follows the junction and empties the main checkout's `node_modules`. Every gate on the machine then fails with `ERR_MODULE_NOT_FOUND` for `playwright`, which reads as the running workers' own code breaking. It has happened twice. The first time was on 2026-09-10, with `--force` (`docs/devlog/detailed/2026-09-10-adaptive-study-and-pages.md`). The second was on 2026-09-25 at about 06:20 UTC, when the coordinator removed seven finished worktrees after a check reported no junction in any of them.
Measured: that check was a PowerShell `Test-Path` written inside a bash double-quoted string. It printed "absent" for a real junction, because the quoting mangled the path, and it still said "absent" on a scratch junction afterwards. On the same junction, `[ -L <path> ]` in Git Bash said "link", and `find <worktree> -type l` listed it. `npm ci` in the main checkout restored `node_modules` in about a second; the browsers under `%LOCALAPPDATA%\ms-playwright` were not touched.
Gate, owed: a worktree-removal helper, the one way a session here removes a worktree. It runs `find <worktree> -type l`, refuses while that finds anything, and names each link and the `cmd //c rmdir <path>` that removes the link alone. Its red proof is a scratch worktree with a junction in it.
